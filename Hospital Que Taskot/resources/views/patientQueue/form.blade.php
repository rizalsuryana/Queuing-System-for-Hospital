...
<section class="section">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <label for="queueQuota" class="form-label">
                                {{-- menghitung sisa di controller --}}
                                <h6> Terisi : {{ $sisaPatient }}</h6>
                            </label>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-12">
                            <label for="queueQuota" class="form-label">
                                <h6> Sisa : {{ $patientQueue->quota-$sisaPatient }}</h6>
                            </label>
                        </div>
                    </div>
                    <!-- <div class="row">
                        <div class="col-12">
                            <a href="{{route('patientQueue')}}" class="btn btn-primary">Kembali</a>

                            <form action="{{ route('exportDetail.pdf') }}" method="GET" style="margin-top: 6px;">
                                <input type="hidden" id="custId" name="id" value="{{$id}}">
                                    <button type="submit" class="btn btn-primary text-left" value="Tidak datang">Export PDF</button>     
                                </form>   
    ...
    {{-- <div class="col-6">
        <form action="{{ route('exportDetail.pdf') }}" method="GET">
            <input type="hidden" id="custId" name="id" value="{{$id}}">
            <button type="submit" class="btn btn-primary text-left">Export PDF</button>
        </form>
    </div> --}}
</div>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table id="wai" class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{$pp->patient_name}}</td>
                                    <td>
                                        <a href="#" class="btn btn-sm btn-primary btnAct" data-id="{{$pp->id}}" data-goto="inroom">Lanjutkan</a>
                                        <form action="{{ route('patientQueue.cancelOne') }}" method="GET" style="margin-top: 6px;">
                                            <input type="hidden" id="custId" name="id" value="{{$pp->id}}">
                                            <button type="submit" class="btn btn-primary text-left" style="padding: 5px 10px; font-size: 12px;" value="Tidak datang">Tidak Datang</button>
                                        </form> 
                                    </td>
                                </tr>
                                @endforeach
                        ...
                        {{-- <form action="{{ route('patientQueue.cancel') }}" method="GET">
                        <input type="hidden" id="custId" name="id" value="{{$id}}">
                            <button type="submit" class="btn btn-danger text-left" value="Tidak datang">Tidak Datang</button>     
                        </form>    --}}
                    </div>
                </div>
            </div>
    ...
    <div class="col-12 col-md-4">
        ...
        <div class="col-12 col-md-4">
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title">Tidak Berkunjung</h5>
                    <hr>
                    <div class="table-responsive">
                        <table id="wai" class="table table-striped">
                            <thead>
                            ...
                            <tbody>
                                @foreach ($cancelledPatients as $pp)                                    
                                <tr>
                                    <td>{{$pp->queue_number}}</td>
                                    <td>{{$pp->patient_name}}</td>
                                </tr>
